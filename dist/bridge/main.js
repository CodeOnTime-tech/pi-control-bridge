...
// bridge/loops.ts
function startHeartbeatLoop(config, backend, logger, getDeviceState, onHeartbeat) {
  let stopped = false;
  const tick = async () => {
    if (stopped) return;
...
function startPollerLoop(config, backend, dispatcher, eventSender, logger, getDeviceState) {
  let stopped = false;
  const tick = async () => {
    if (stopped) return;
    const state = getDeviceState();
...
var BridgeRuntime = class {
  ...
  async ensureDeviceRegistered() {
    ...
    this.deviceState = this.backend.toDeviceState(result, fingerprint, saved ?? void 0);
